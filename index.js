...
app.get('/generate_keys.html', (req, res) => res.sendFile(__dirname + '/generate_keys.html'))
app.get('/tx_builder.html', (req, res) => res.sendFile(__dirname + '/tx_builder.html'))
app.get('/keyslist.html', (req, res) => res.sendFile(__dirname + '/keyslist.html'))
...
app.get('/api/keys/all', function(req, res) {
    ...
    if (slot.flags & graphene.SlotFlag.TOKEN_PRESENT) {
        ...
        for (i = 0; i < keys.length; i++) {
            try {
                puplicKey = decodeECPointToPublicKey(keys.items(i).getAttribute({ pointEC: null }).pointEC)
                pkstr = keys.items(i).getAttribute({ keyGenMechanism: null }).keyGenMechanism
...
app.post('/api/keys/generate', function(req, res) {
    ...
    var gkeys = session.generateKeyPair(graphene.KeyGenMechanism.ECDSA, {
        label: keylabel,
    ...
    puplicKey = decodeECPointToPublicKey(gkeys.publicKey.getAttribute({ pointEC: null }).pointEC)
...
app.post('/api/tx/generator', function(req, res) {
    ...
    for (i = 0; i < allPkeys.length; i++) {
        if (allPkeys.items(i).getAttribute({ label: null }).label == "EthreAddrees1") {
...
//------------------------------------------------------------------------
app.listen(8090, () => console.log('Web app listening at http://localhost:8090'))